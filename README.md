...
这不是说 Claude Code 不能用，而是不要把长期积累的开发上下文锁死在单一工具里。现在模型服务、订阅策略、额度规则和产品边界变化都很快；平台也天然倾向于把用户留在自己的生态中。对开发者来说，项目规则、skills、commands、memory 和历史决策本质上是自己的工作资产，应该能被审阅、备份和迁移。
